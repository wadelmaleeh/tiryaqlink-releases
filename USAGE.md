...
git commit -m "Add build artifacts for v1.0.3"
git tag v1.0.3
git push origin main --tags
```

The tag must match `^v[0-9]+\.[0-9]+\.[0-9]+$` (e.g. `v1.0.3`).

## 3. Workflow creates the release

Pushing the `v1.0.3` tag triggers `.github/workflows/release.yml`, which validates that `./dist/TiryaQLink-Setup-1.0.3.exe` and `./dist/TiryaQLink-1.0.3.apk` exist in the tagged commit, then creates a GitHub Release attaching both files with those exact filenames.
