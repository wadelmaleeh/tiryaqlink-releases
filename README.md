...
Hosts the Windows release installer and Android APK for the TiryaQLink app. Every release is created automatically by the [GitHub Actions workflow](.github/workflows/release.yml) when a `v<version>` tag (e.g. `v1.0.5`) is pushed.
...
> Replace `1.0.5` with the version you are downloading. The `latest` part of each URL always resolves to the newest tagged release, so these links never need to be updated when a new version ships.
